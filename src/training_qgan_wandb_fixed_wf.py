...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--wasserstein-eval-samples", "--wasserstein_eval_samples", type=int, default=1500,
                        help="Anzahl Samples fuer Wasserstein- und Korrelations-"
                             "Zwischenauswertung. Korrelationen brauchen mehr Samples "
                             "(6x6-Matrix), daher 1500 Default (war 500 vor Composite).")
    parser.add_argument("--composite-norm-wasserstein", "--composite_norm_wasserstein",
                        type=float, default=0.023,
                        help="Normalisierungs-Konstante fuer Wasserstein im Composite-Score. "
                             "Default 0.023 = breezy-sweep-24 (bisher beste Marginal-Performance).")
    parser.add_argument("--composite-norm-frobenius", "--composite_norm_frobenius",
                        type=float, default=1.18,
                        help="Normalisierungs-Konstante fuer Frobenius im Composite-Score. "
                             "Default 1.18 = breezy-sweep-24 (Korrelations-Frobenius dieses Runs).")
...
def evaluate_wasserstein(
    ...
    real_corr_cached: Optional[np.ndarray] = None,
    composite_norm_wasserstein: float = 0.023,
    composite_norm_frobenius: float = 1.18,
) -> Dict[str, float]:
    """
    Schnelle Zwischenauswertung der statistischen Generator-Qualitaet.

    Berechnet:
    - Wasserstein-Distanz pro Kante (Marginale, 1. Ordnung)
    - Frobenius-Distanz der Korrelationsmatrix (2. Ordnung)
    - Composite-Score: normalisierte Summe beider Metriken

    Der Composite-Score nutzt die Referenz-Werte von breezy-sweep-24
    (mean_wasserstein=0.023, frobenius_correlations=1.18) zur Normalisierung.
    Damit haben beide Komponenten beim Referenz-Run jeweils den Wert 1.0,
    und Composite=2.0 entspricht "wie breezy-24". Composite < 2.0 = besser.

    WICHTIG: Diese Funktion isoliert den RNG-Zustand vollstaendig.
    Sie speichert torch und numpy RNG-Zustaende vor der Ausfuehrung und
    stellt sie danach wieder her. Damit beeinflusst die Wasserstein-
    Zwischenauswertung das Trainings-RNG NICHT — der Trainingsverlauf ist
    identisch zu einem Lauf ohne diese Auswertung (bei gleichem Seed).
    ...
    Parameters:
        real_corr_cached: Vorberechnete Real-Korrelationsmatrix (6x6). Wenn None,
            wird sie aus dem aktuellen Real-Subset berechnet (weniger genau).
        composite_norm_wasserstein: Wert fuer Wasserstein-Normalisierung
            (Default: 0.023 = breezy-sweep-24 Referenz).
        composite_norm_frobenius: Wert fuer Frobenius-Normalisierung
            (Default: 1.18 = breezy-sweep-24 Referenz).

    Returns dict mit:
        w_<edge>:        Wasserstein-Distanz pro Kante (6 Eintraege)
        w_mean:          mittlere Wasserstein-Distanz
        frobenius_corr:  Frobenius-Distanz der Korrelationsmatrizen
        composite:       normalisierter Composite-Score (kleiner = besser)
    """
    if not SCIPY_AVAILABLE:
        return {}

    # ---- RNG-Zustand speichern (kritisch fuer Reproduzierbarkeit) ----
    saved_torch_rng = torch.get_rng_state()
    saved_torch_cuda_rng = None
    if torch.cuda.is_available():
        saved_torch_cuda_rng = torch.cuda.get_rng_state_all()
    saved_numpy_rng = np.random.get_state()

    try:
        # Real-Stichprobe (nutzt eigenen rng, nicht den globalen)
        ...
        try:
            ...
            # Generator-Modus zuruecksetzen, falls vorher train
            if was_training:
                gen.train()

        fakes = np.concatenate(outputs, axis=0)[:n_samples]

        # ---- Wasserstein-Distanzen (1. Ordnung) ----
        per_edge_w = {}
        for i, name in enumerate(EDGE_NAMES):
            per_edge_w[f"w_{name}"] = float(wasserstein_distance(real_subset[:, i], fakes[:, i]))
        w_mean = float(np.mean(list(per_edge_w.values())))

        # ---- Korrelations-Frobenius-Distanz (2. Ordnung) ----
        # Real-Korrelation: bevorzugt aus dem ganzen Datensatz (cached), sonst aus Subset
        if real_corr_cached is not None:
            corr_real = real_corr_cached
        else:
            corr_real = np.corrcoef(real_subset.T)
        # Fake-Korrelation: aus den frisch generierten Samples
        # NaN moeglich, wenn eine Kante komplett konstant ist (Mode Collapse extrem)
        with np.errstate(invalid="ignore"):
            corr_fake = np.corrcoef(fakes.T)
        # NaN durch 0 ersetzen (passiert nur bei degenerierten Verteilungen)
        corr_fake = np.nan_to_num(corr_fake, nan=0.0)
        frobenius_corr = float(np.linalg.norm(corr_real - corr_fake))

        # ---- Composite-Score (normalisiert) ----
        # composite = w_mean / norm_w + frobenius / norm_f
        # Beim Referenz-Run (breezy-24) ist composite = 1.0 + 1.0 = 2.0
        ...
        # ---- RNG-Zustand wiederherstellen ----
        torch.set_rng_state(saved_torch_rng)
        if saved_torch_cuda_rng is not None:
            torch.cuda.set_rng_state_all(saved_torch_cuda_rng)
...
def main() -> None:
    ...
    # ---- Best-Fake-Score-G-Tracking ueber das gesamte Training ----
    # Hoechster beobachteter fake_score_g (Generator Fooling Success).
    # Bei der Maximierung als Sweep-Metrik bevorzugt man den besten erreichten
    # Wert ueber das Training (analog zu best_wasserstein bei der Minimierung).
    best_fake_score_g = float("-inf")
    best_fake_score_g_step = 0

    # ---- Mean-Fake-Score-G-Tracking ueber das gesamte Training ----
    # Mittelwert von fake_score_g ueber ALLE Trainings-Steps.
    # Robuster als best_fake_score_g, weil einzelne Glueck-Spikes nur 1/5000
    # zum Mittelwert beitragen koennen. Bevorzugt konsistent gute Generatoren
    # gegenueber Configs mit Mode-Collapse-Spikes.
    # Implementation: laufende Summe + Zaehler, kein Speichern der vollen Liste.
    fake_score_g_sum = 0.0
    fake_score_g_count = 0

    # ---- Best-Composite-Tracking ueber das gesamte Training ----
    # Composite-Score: normalisierter Mix aus Wasserstein (1. Ordnung) und
    # Frobenius-Korrelations-Distanz (2. Ordnung). Niedriger = besser.
    # Ziel-Metrik fuer Sweep 3.
    ...
    # von Stichproben-Rauschen abhaengt.
    if SCIPY_AVAILABLE:
        real_corr_full = np.corrcoef(real_data.T)
        print(f"[INFO] Real-Korrelationsmatrix vorab berechnet (Shape: {real_corr_full.shape})")
        print(f"[INFO] Composite normalisiert: wasserstein/{args.composite_norm_wasserstein} + "
              f"frobenius/{args.composite_norm_frobenius}")
    else:
        real_corr_full = None
    ...
    for step in range(1, args.steps + 1):
        ...
        # ---- Best-Fake-Score-G-Tracking pro Step ----
        # Analog zu best_wasserstein: maximiere ueber das gesamte Training.
        current_fake_score_g = float(metrics["fake_score_g"])
        if current_fake_score_g > best_fake_score_g:
            best_fake_score_g = current_fake_score_g
            best_fake_score_g_step = step
        wandb_log_dict["scores/fake_gen_best_so_far"] = best_fake_score_g
        wandb_log_dict["scores/fake_gen_best_step"] = best_fake_score_g_step

        # ---- Mean-Fake-Score-G-Tracking pro Step ----
        # Laufender Mittelwert: jeder Step zaehlt gleich.
        fake_score_g_sum += current_fake_score_g
        fake_score_g_count += 1
        mean_fake_score_g = fake_score_g_sum / fake_score_g_count
        wandb_log_dict["scores/fake_gen_mean_so_far"] = mean_fake_score_g

        # ---- Wasserstein- + Korrelations- + Composite-Zwischenauswertung alle N Steps ----
        if (
            ...
            w_metrics = evaluate_wasserstein(
                ...
                device=device,
                rng=eval_rng,
                real_corr_cached=real_corr_full,
                composite_norm_wasserstein=args.composite_norm_wasserstein,
                composite_norm_frobenius=args.composite_norm_frobenius,
            )
            if w_metrics:
                for k, v in w_metrics.items():
                    wandb_log_dict[f"wasserstein/{k}"] = v

                # Best-Tracking Wasserstein
                ...
                # Best-Tracking Composite (Sweep-3-Ziel)
                current_composite = w_metrics["composite"]
                if current_composite < best_composite:
                    best_composite = current_composite
                    best_composite_step = step
                    best_composite_w = current_w
                    best_composite_f = w_metrics["frobenius_corr"]
                wandb_log_dict["composite/best_so_far"] = best_composite
                wandb_log_dict["composite/best_step"] = best_composite_step
    ...
    if use_wandb:
        wandb_summary = {
            ...
            "best_fake_score_g": float(best_fake_score_g),
            "best_fake_score_g_step": int(best_fake_score_g_step),
            "mean_fake_score_g": float(mean_fake_score_g),
    ...
    print(f"[BEST] Fake-Score-G (max ueber Training): {best_fake_score_g:.4f} @ step {best_fake_score_g_step}")
