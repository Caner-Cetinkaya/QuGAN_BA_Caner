"""
Hybrid GAN: QGenerator + classical Discriminator — GROUND ZERO VERSION

Diese Variante ist die methodische Baseline:
- KEIN Minibatch Discrimination
- KEIN trainierbarer Post-Processing-Adapter
- Label Smoothing aktiv (gilt für D UND G — Generator-Target = 0.9)
- (z+1)/2-Mapping erlaubt (bijektive lineare Range-Konvertierung, parameterfrei)

Ziel: prüfen, ob der reine Quantum-Generator + klassischer Discriminator
ohne Hilfs-Mechanismen ein sinnvolles Lernverhalten zeigt.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ground Zero hybrid GAN: QGEN + cDISC, no MBD, no post-adapter."
    )
...
def make_training_plot(csv_path: Path, out_path: Path, smooth_window: int = 10) -> None:
    ...
    fig.suptitle(
        f"Ground Zero Hybrid GAN: QGEN + cDISC (Smoothed over {smooth_window} steps)",
...
def make_per_edge_distribution_plot(real_batch: np.ndarray, fake_batch: np.ndarray, out_path: Path) -> None:
    ...
    fig.suptitle("Ground Zero Hybrid GAN: Per-edge distributions", fontsize=16, fontweight="bold")
...
def main() -> None:
    ...
    if args.output_dir is None:
        ...
        output_dir = Path(f"logs/ground_zero_qgen_cdisc_{timestamp}")
    else:
