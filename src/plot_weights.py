...
for col in ["step", "w1", "w2", "w3"]:
    df[col] = pd.to_numeric(df[col], errors="coerce")
df = df.dropna(subset=["step", "w1", "w2", "w3"])

plt.figure()
plt.plot(df["step"], df["w1"], label="w1")
plt.plot(df["step"], df["w2"], label="w2")
plt.plot(df["step"], df["w3"], label="w3")
plt.xlabel("step")
plt.ylabel("weight")
...
out_png = os.path.join(os.path.dirname(path), "plot_weights.png")
