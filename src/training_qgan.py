...
def _disc_probs_from_edges(disc: QDiscriminator, weights, edges_batch):
    ...
    # QNode now returns one PauliZ expval per qubit
    z = disc.circuit(edges_batch, weights)

    # If multiple expvals are returned, stack them into (..., n_qubits)
    #if isinstance(z, (list, tuple)):
    #    z = pnp.stack(z, axis=-1)

    # Average over qubit readouts to get a single score in [-1, 1]
...
def train_discriminator_step(disc, batch_real, batch_fake):
    """Single discriminator step with separated real/fake losses."""
    ...
    def loss_inner(weights):
        real_preds = _disc_probs_from_edges(disc, weights, batch_real_p)
        fake_preds = _disc_probs_from_edges(disc, weights, batch_fake_p)
    ...
    # getrennte Logging-Werte nach dem Update
    real_preds_after = _disc_probs_from_edges(disc, disc.weights, batch_real_p)
    fake_preds_after = _disc_probs_from_edges(disc, disc.weights, batch_fake_p)
...
def train_generator_step(disc, gen, noise_batch):
    """Single generator training step using analytic gradients via qml.grad.
    1. gen.weights differentiable (requires_grad=True)
    2. disc.weights als konstantes Array (ohne grad) → freeze 
    3. loss_inner(gen_weights):
        fake samples = _gen_edges_from_noise(...)
        scores = _disc_probs_from_edges(disc, disc.weights, batch_fake)
        targets = ones (G will D(fake)=1)
        loss = _loss_fn(scores, targets)
    4. qml.grad -> grad
    5. Update: gen.weights = gen.weights - lr * grad
    6. Für Logging wird fake batch nochmal als np erzeugt
    
    G lernt D zu täuschen; D bleibt fest; Update nur auf gen.weights
    """

    # Ensure weights are differentiable
    gen.weights = pnp.array(gen.weights, dtype=float, requires_grad=True)
    disc.weights = pnp.array(disc.weights, dtype=float)  # treat as constant for generator step

    def loss_inner(gen_weights):
        batch_fake = _gen_edges_from_noise(gen, gen_weights, noise_batch)
        scores = _disc_probs_from_edges(disc, disc.weights, batch_fake)
        # non-saturating generator objective: push D(fake) -> 1
    ...
    # For logging (non-differentiable numpy is fine here)
...
def main():
    ...
    for step in range(1, TRAINING_STEPS + 1):
        ...
        fake_scores_disc = np.array(_disc_probs_from_edges(disc, disc.weights, batch_fake_gen), dtype=float)
        fake_scores_gen = np.array(_disc_probs_from_edges(disc, disc.weights, batch_fake_final), dtype=float)
