"""
Hybrid GAN: QGenerator + classical Discriminator — EXPERIMENT 2: PROBS OUTPUT

Diese Variante stellt nur den Quantum-Output um (Emily-Style):
- qml.probs(wires=range(6))[:6] statt PauliZ-Expectation
- Output wird durch Summe normalisiert (lebt im 5-dim Simplex)
- Real-Daten werden ENTSPRECHEND normalisiert (sum upper-triangle = 1)
- Alles andere wie Baseline: KEIN MBD, KEIN Post-Adapter, Label Smoothing aktiv

Ziel: pruefen, ob die Simplex-Repraesentation mit eingebauten Korrelationen
zwischen den Kanten dem Generator hilft, die echte Datenstruktur besser zu lernen.

WICHTIG: Real-Daten sind hier ANDERS normalisiert als in Baseline/EXP1.
Direkte Verteilungsvergleiche zwischen EXP2 und Baseline sind dadurch
NICHT moeglich. Vergleichbar sind nur die qualitativen Trainings-Metriken
und die Wasserstein-Distanzen JE Setup separat.
"""
...
# =============================================================================
# Classical Discriminator — KEIN Minibatch Discrimination
# =============================================================================
...
# =============================================================================
# Quantum Generator — qml.probs[:6] + Simplex-Normalisierung (Emily-Style)
# =============================================================================
class QGenerator(nn.Module):
    """
    6-Qubit Quantum-Generator mit echtem Batching, Emily-Style Output.

    Output ist qml.probs(wires=range(6))[:6], normalisiert auf Summe = 1.
    Damit lebt der Output im 5-dim Simplex (jede Komponente in [0,1],
    Summe ueber alle 6 Komponenten = 1).

    Input  forward(noise_batch): (batch, 6) oder (6,)
    Output:                      (batch, 6) oder (6,), summiert pro Sample zu 1
    """
    ...
    def __init__(self, n_layer: int = 4, init_std: float = 0.1, seed: int = 0):
        ...
        def circuit(noise_batch: torch.Tensor, weights: torch.Tensor):
            ...
            # Volle Wahrscheinlichkeitsverteilung ueber 64 Basiszustaende
    ...
    def forward(self, noise_batch: torch.Tensor) -> torch.Tensor:
        ...
        probs = self.circuit(noise_batch, self.weights)  # (batch, 64)
        probs = probs.to(dtype=torch.float32)

        # Erste 6 Basiszustaende rausziehen
        first6 = probs[..., :6]  # (batch, 6)

        # Auf Summe = 1 normalisieren (Simplex)
        # Kleines eps, falls Summe extrem klein wird (numerisch sicher)
        out = first6 / (first6.sum(dim=-1, keepdim=True) + 1e-12)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="EXP2: Hybrid GAN: QGEN (probs[:6]+Simplex) + cDISC, ohne MBD, ohne post-adapter."
    )
...
def load_real_data(valid_tuples_path: str) -> np.ndarray:
    ...
    # EXP2: Simplex-Normalisierung (Emily-Style): jede Zeile auf Summe = 1
    # Statt: real = np.clip(real / MAX_EDGE_LENGTH_KM, 0.0, 1.0)
    # Die echten Distanzen werden so umgerechnet, dass die 6 Kanten eines Graphs
    # zusammen 1 ergeben -- gleiche Repraesentation wie der Generator-Output.
    row_sums = real.sum(axis=1, keepdims=True)
    # Sicherheit: keine Division durch 0 (sollte bei echten Distanzen nie passieren)
...
def make_training_plot(csv_path: Path, out_path: Path, smooth_window: int = 10) -> None:
    ...
    fig.suptitle(
        f"EXP2 (probs) Hybrid GAN: QGEN + cDISC (Smoothed over {smooth_window} steps)",
...
def make_per_edge_distribution_plot(real_batch: np.ndarray, fake_batch: np.ndarray, out_path: Path) -> None:
    ...
    fig.suptitle("EXP2 (probs) Hybrid GAN: Per-edge distributions", fontsize=16, fontweight="bold")
...
def main() -> None:
    ...
    if args.output_dir is None:
        ...
        output_dir = Path(f"logs/exp2_probs_qgen_cdisc_{timestamp}")
    else:
    ...
    print(f"[INFO] === EXPERIMENT 2: PROBS RUN ===")
