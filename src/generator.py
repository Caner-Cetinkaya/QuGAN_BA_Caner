...
1. NOISE EMBEDDING (einmalig):
   ...
   - Qubit-Zuordnung: Q0←noise[0], Q1←noise[1], ..., Q5←noise[5]
    - Erwartet kleine, nicht-periodische Winkelinputs; volle Phasenintervalle [0, 2pi]
      zentrieren die Einzelqubit-Wahrscheinlichkeiten sonst um 0.5
...
class QGenerator:
    def __init__(self, n_layer: int, seed: int | None = 0):
        ...
        def circuit(noise_vector, weights):
            ...
            measurements = [qml.probs(wires=i) for i in range(self.n_qubits)]
            return measurements
    ...
    def _to_edge_length(self, probs_values) -> np.ndarray:
        probs_arr = pnp.array(probs_values, dtype=float)

        # Einzelnes Sample: shape (6, 2)
        if probs_arr.ndim == 2 and probs_arr.shape == (self.n_qubits, 2):
            return probs_arr[:, 1]

        # Batch-Fall kommt normalerweise schon passend vorbereitet hier an
        # -> shape (B, 6, 2)
        if probs_arr.ndim == 3 and probs_arr.shape[1] == self.n_qubits and probs_arr.shape[2] == 2:
            return probs_arr[:, :, 1]

        raise ValueError(f"Unerwartete probs shape in _to_edge_length: {probs_arr.shape}")    
    ...
    def forward(self, noise_vector: np.ndarray) -> np.ndarray:
            ...
            edges_normalized = self._to_edge_length(probs_values)
            return np.asarray(edges_normalized, dtype=float)

    """
    def _to_edge_length(self, z_values: np.ndarray) -> np.ndarray:
        z = pnp.array(z_values, dtype=float)
        raw = self.output_gain * z + self.output_bias
        p = self._sigmoid(raw)
        return p
    """
    """
    ...
    def forward(self, noise_vector: np.ndarray) -> np.ndarray:
        ...
        z_values = self.circuit(noise_vector, self.weights)
    ...
    def batch_forward(self, noise_batch: np.ndarray, weights=None) -> np.ndarray:
        ...
        """
        passed_weights = weights is not None
        if weights is None:
        ...
        z_values = self.circuit(noise_batch, weights)

        if isinstance(z_values, (list, tuple)):
            z_arr = pnp.stack(z_values, axis=1)  # (B, 6)
        else:
            z_arr = pnp.array(z_values)
            if z_arr.ndim == 3 and z_arr.shape[0] == self.n_qubits:
                z_arr = pnp.transpose(z_arr, (1, 0, 2))

            #if z_arr.ndim == 2 and z_arr.shape[0] == 6:
            #    z_arr = z_arr.T

        # DEBUG NUR AUSSERHALB DES GRAD-PFADS
        if not passed_weights:
            z_np = np.array(z_arr)
            #print("[DEBUG] raw z shape:", z_np.shape)
            #print("[DEBUG] raw z mean per qubit:", np.round(z_np.mean(axis=0), 3))
            #print("[DEBUG] raw z std  per qubit:", np.round(z_np.std(axis=0), 3))
            #print("[DEBUG] raw z first sample:", np.round(z_np[0], 3))

        edges = self._to_edge_length(z_arr)

        if not passed_weights:
            edges_np = np.array(edges)
            #print("[DEBUG] edges first sample:", np.round(edges_np[0], 3))
