...
RUNTIME_SEED = _get_env_int("CGAN_SEED", SEED)
RUNTIME_BATCH_SIZE = _get_env_int("CGAN_BATCH_SIZE", BATCH_SIZE)
RUNTIME_TRAINING_STEPS = _get_env_int("CGAN_TRAINING_STEPS", TRAINING_STEPS)
RUNTIME_DISC_STEPS_PER_GEN = _get_env_int("CGAN_DISC_STEPS_PER_GEN", int(DISC_STEPS_PER_GEN))
RUNTIME_DISC_LEARNING_RATE = _get_env_float(
    "CGAN_DISC_LEARNING_RATE",
    DISC_LEARNING_RATE if DISC_LEARNING_RATE is not None else LEARNING_RATE,
)
RUNTIME_GEN_LEARNING_RATE = _get_env_float(
    "CGAN_GEN_LEARNING_RATE",
    GEN_LEARNING_RATE if GEN_LEARNING_RATE is not None else LEARNING_RATE,
)
RUNTIME_DISC_WARMUP_STEPS = _get_env_int("CGAN_DISC_WARMUP_STEPS", DISC_WARMUP_STEPS)
RUNTIME_GRAD_CLIP_NORM = _get_env_float("CGAN_GRAD_CLIP_NORM", 100.0)
RUNTIME_LABEL_REAL = _get_env_float("CGAN_LABEL_REAL", LABEL_REAL)
RUNTIME_LABEL_FAKE = _get_env_float("CGAN_LABEL_FAKE", LABEL_FAKE)
RUNTIME_MAX_EDGE_LENGTH_KM = _get_env_float("CGAN_MAX_EDGE_LENGTH_KM", MAX_EDGE_LENGTH_KM)
RUNTIME_LOSS_TYPE = _get_env_str("CGAN_LOSS_TYPE", LOSS_TYPE)
RUNTIME_LATENT_DISTRIBUTION = _get_env_str("CGAN_LATENT_DISTRIBUTION", "uniform").lower()
RUNTIME_RUN_NAME = _get_env_str("CGAN_RUN_NAME", "")
RUNTIME_LOG_ROOT = _get_env_str("CGAN_LOG_ROOT", "logs")
...
LATENT_DIM = 6
GRAD_CLIP_NORM = RUNTIME_GRAD_CLIP_NORM
# LATENT_DISTRIBUTION = "uniform"  # set to "normal" if you explicitly want torch.randn
LATENT_DISTRIBUTION = RUNTIME_LATENT_DISTRIBUTION
...
# LOSS_FN = GANLoss(RUNTIME_LOSS_TYPE)
...
def create_batch_real(cities, batch_size, cache, rng_obj):
    ...
    while len(batch) < batch_size and attempt < max_retries:
        ...
        if not success:
            continue
        normalized_unclipped = edges_km / RUNTIME_MAX_EDGE_LENGTH_KM
...
def train_discriminator_step(generator, discriminator, optimizer_d, cities, cache, rng_obj, batch_size):
    ...
    target_real = torch.full((batch_size, 1), float(RUNTIME_LABEL_REAL), device=DEVICE)
    target_fake = torch.full((batch_size, 1), float(RUNTIME_LABEL_FAKE), device=DEVICE)
    ...
    disc_loss_real = LOSS_FN(real_probs, target_real)
    disc_loss_fake = LOSS_FN(fake_probs, target_fake)
...
def main():
    ...
    optimizer_g = optim.Adam(
        generator.parameters(),
        lr=RUNTIME_GEN_LEARNING_RATE,
        betas=(0.5, 0.999),
    )
    optimizer_d = optim.Adam(
        discriminator.parameters(),
        lr=RUNTIME_DISC_LEARNING_RATE,
        betas=(0.5, 0.999),
    )

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_stem = f"cgan_comparable_{timestamp}"
    if RUNTIME_RUN_NAME:
        run_stem = f"{run_stem}_{RUNTIME_RUN_NAME}"
    log_dir = Path(RUNTIME_LOG_ROOT) / run_stem
    ...
    config_dict = {
        ...
        "training_steps": RUNTIME_TRAINING_STEPS,
        "disc_steps_per_gen": RUNTIME_DISC_STEPS_PER_GEN,
        "disc_learning_rate": RUNTIME_DISC_LEARNING_RATE,
        "gen_learning_rate": RUNTIME_GEN_LEARNING_RATE,
        "seed": RUNTIME_SEED,
        "loss_type": RUNTIME_LOSS_TYPE,
        "disc_warmup_steps": RUNTIME_DISC_WARMUP_STEPS,
        "device": str(DEVICE),
        "model_type": "classical_gan_comparable_to_qgan",
        "discriminator_outputs": "logits",
        "gradient_clip_norm": GRAD_CLIP_NORM,
        "latent_distribution": LATENT_DISTRIBUTION,
        "label_real": RUNTIME_LABEL_REAL,
        "label_fake": RUNTIME_LABEL_FAKE,
        "max_edge_length_km": RUNTIME_MAX_EDGE_LENGTH_KM,
    }
    ...
    print(f"[INFO] Starting comparable training for {RUNTIME_TRAINING_STEPS} steps...")
    print(f"[CONFIG] LOSS_TYPE={RUNTIME_LOSS_TYPE} (mapped to qGAN-style semantics)")
    print(f"[CONFIG] LATENT_DISTRIBUTION={LATENT_DISTRIBUTION}")
    print(
        "[CONFIG] "
        f"seed={RUNTIME_SEED}, "
        f"batch_size={RUNTIME_BATCH_SIZE}, "
        f"d_steps={RUNTIME_DISC_STEPS_PER_GEN}, "
        f"d_lr={RUNTIME_DISC_LEARNING_RATE}, "
        f"g_lr={RUNTIME_GEN_LEARNING_RATE}, "
        f"warmup={RUNTIME_DISC_WARMUP_STEPS}, "
        f"grad_clip={GRAD_CLIP_NORM}"
    )

    for step in range(1, RUNTIME_TRAINING_STEPS + 1):
        d_runs = []

        for _ in range(RUNTIME_DISC_STEPS_PER_GEN):
          d_runs.append(
        ...
        if step > RUNTIME_DISC_WARMUP_STEPS:
