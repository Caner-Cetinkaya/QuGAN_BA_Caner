"""
Hybrid GAN: QGenerator + classical Discriminator — BASELINE VERSION
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Baseline hybrid GAN: QGEN + cDISC, no MBD, no post-adapter."
    )
...
def make_training_plot(csv_path: Path, out_path: Path, smooth_window: int = 10) -> None:
    ...
    fig.suptitle(
        f"Baseline Hybrid GAN: QGEN + cDISC (Smoothed over {smooth_window} steps)",
...
def make_per_edge_distribution_plot(real_batch: np.ndarray, fake_batch: np.ndarray, out_path: Path) -> None:
    ...
    fig.suptitle("Baseline Hybrid GAN: Per-edge distributions", fontsize=16, fontweight="bold")
...
def main() -> None:
    ...
    if args.output_dir is None:
        ...
        output_dir = Path(f"logs/baseline_qgen_cdisc_{timestamp}")
    else:
